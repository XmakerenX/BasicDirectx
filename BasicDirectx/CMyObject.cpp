...
CMyObject::CMyObject(void)
{
	...
	XMStoreFloat4x4(&m_mtxWorld, I);
	XMStoreFloat4x4(&m_mtxRot, I);
	XMStoreFloat4x4(&m_mtxScale, I);
...
DirectX::XMMATRIX  CMyObject::getWorldMatrix(DirectX::XMMATRIX& worldMatrix)
{
	//D3DXMatrixRotationX(&rot,m_rotAngle);
	DirectX::XMMATRIX world = XMLoadFloat4x4(&m_mtxWorld);
	DirectX::XMMATRIX rotaion = XMLoadFloat4x4(&m_mtxRot);
	DirectX::XMMATRIX scale = XMLoadFloat4x4(&m_mtxScale);
	worldMatrix = scale * rotaion * world;
	return worldMatrix;
...
void CMyObject::setPos(DirectX::XMFLOAT3 newPos)
{
...
void CMyObject::setRotAngels(DirectX::XMFLOAT3 rotAngels)
{
	DirectX::XMMATRIX  mtxRotX;
	DirectX::XMMATRIX  mtxRotY;
	DirectX::XMMATRIX  mtxRotZ;

	m_rotAngles = rotAngels;
	mtxRotX = DirectX::XMMatrixRotationX(m_rotAngles.x);
	mtxRotY = DirectX::XMMatrixRotationX(m_rotAngles.y);
	mtxRotZ = DirectX::XMMatrixRotationX(m_rotAngles.z);

	mtxRotX = mtxRotX * mtxRotY * mtxRotZ;
	//TODO: check if transposing the matrix is needed!!
	DirectX::XMStoreFloat4x4(&m_mtxRot, mtxRotX);
}
